...
void Custom::init()
{
    // Get control command from topic pd_plustau_targets
...
void Custom::RobotControl()
{
    ...
    // Sao chép dữ liệu từ bộ điều khiển không dây của robot vào biến _keyData để sử dụng cho điều khiển
    memcpy(&_keyData, &state.wirelessRemote[0], 40);

    // Đọc dữ liệu từ bộ điều khiển -> joystick
    ...
    // Đọc dữ liệu từ bộ điều khiển -> button
    ...
    // Các dữ liệu vị trí, vận tốc và mô-men xoắn từ các khớp của robot được cập nhật vào cấu trúc joint_state_simple
    ...
    // Thiết lập lệnh điều khiển cho các khớp
    for(int i = 0; i < 12; i++){
