...
class RobotPIDController:
    ...
    def calculate_relative_position(self):
        try:
            ...
            tran_robot, rot_robot = self.tf_listener.lookupTransform(ROBOT_TF_FRAME, 'world', now)
            tran_target, rot_target = self.tf_listener.lookupTransform(TARGET_TF_FRAME, 'world', now)
            print('tran_robot:', tran_robot)
            print('tran_target:', tran_target)
